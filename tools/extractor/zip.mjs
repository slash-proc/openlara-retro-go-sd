...
//
// Names may also contain "/". OpenLara reads its levels from a folder whose
// name is compiled into the binary, so an install here is genuinely two
// directories deep -- homebrews/OpenLara.bin beside homebrews/openlara/*.PKD --
// and a flat zip would hand the user a pile of files and a layout to work out
// themselves. Every segment is still validated: no empty segment, no "..", no
// absolute path, no backslash. A path is the one place a crafted manifest
// could aim a write outside the folder the user extracts into, so the check is
// on the writer rather than on whoever calls it.
...
/**
 * Builds a zip from `[{ name, data }]` and returns it as a Blob. A name may be
 * a "/"-separated path.
 *
...
export async function makeZip(entries, { date = new Date() } = {}) {
  ...
  for (const e of entries) {
    ...
    if (e.name.includes("\\")) {
      throw new Error(`zip entry name must use "/" as its separator: ${e.name}`);
    }
    for (const seg of e.name.split("/")) {
      if (seg === "" || seg === "." || seg === "..") {
        throw new Error(`zip entry name has an unusable path segment: ${e.name}`);
      }
    }
    // Case-folded, because the card this ends up on folds case: two entries
    // that differ only in case unpack as two files here and become one there,
    // which is a silent overwrite at the last possible moment.
    const key = e.name.toLowerCase();
    if (seen.has(key)) throw new Error(`duplicate zip entry: ${e.name}`);
    seen.add(key);
  }

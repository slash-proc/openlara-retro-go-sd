...
/* Homebrew hook: keep the PCM ring fed during polling decode. */
                                   ...
                                   uint8_t *pDataOutMCU, uint32_t OutDataLength, uint32_t Timeout)
{
  ...
  if (hjpeg->State == HAL_JPEG_STATE_READY)
  {
    ...
    /*JPEG data processing : In/Out FIFO transfer*/
    uint32_t last_poll = tickstart;
    while ((JPEG_Process(hjpeg) == JPEG_PROCESS_ONGOING))
    {
      /* Keep the video player's PCM ring fed; this loop otherwise starves
       * SAI for the whole decode (underrun → silence holes). */
      if ((HAL_GetTick() - last_poll) != 0UL)
      {
        video_jpeg_poll();
